...
sub install_package {
    ...
    foreach my $prereq_category (keys %{$prereqs}) {
        ...
        foreach my $prereq_name (keys %{$runtime_prereqs}) {
            ...
            $self->_install_prereq($prereq_category, $prereq_name, $prereq_data, $dir, $opts);
        }
    }
...
sub is_installed {
    ...
    if (defined $installer_cache->{$pkg_cat}{$pkg_name}) {
        ...
        if ($version ne $package->version) {
            croak(
                $log->criticalf(
                    "%s=$version already installed. " . 'Cannot install new version: %s', $package->short_name,
                    $package->version,
                )
            );
        }
        ...
        if ($release ne $package->release) {
            croak(
                $log->criticalf(
                    '%s=%s:%s already installed. ' . 'Cannot install new version: %s:%s',
                    $package->short_name, $version, $release, $package->release,
                )
            );
        }
...
sub _mark_as_installed {
    ...
    $installer_cache->{$pkg_cat}{$pkg_name} = [$package->version, $package->release];
}
...
sub _pre_install_checks {
    ...
    if (!$opts->{'skip_prereqs'} && $package->is_bootstrap) {
        croak(
            $log->critical(
                'You are trying to install a bootstrap version of %s.' . ' Please rebuild this package from scratch.',
                $package->full_name,
            )
        );
    }
    ...
    for my $package (@packages) {
        ...
        if ($installed_package) {
            if ($installed_package->full_name ne $package->full_name) {
                ...
                $self->silent
                    or $log->infof('%s already installed, but enabled --force, reinstalling it', $package->full_name);
                push @result, $package;
            }
        } else {
            push @result, $package;
...
sub _set_latest_version_for_undefined {
    ...
    foreach my $package (@packages) {
        if ($package->version && $package->release) {
            ...
            my ($ver, $rel)
                = @{$self->parcel_repo->latest_version_release($package->category, $package->name, $ver_condition)};
