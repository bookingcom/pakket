...
sub execute {
    ...
    my $is_local = +{map {$_ => 1} @{$self->{'opt'}{'is_local'}}};
...
sub _validate_args_add {
    ...
    if (is_arrayref($additional_phase)) {
        exists PAKKET_VALID_PHASES->{$_} or $self->usage_error("Unsupported phase: $_") for @{$additional_phase};
...
sub _validate_args_dependency {
    ...
    defined $opt->{$_} or $self->usage_error("Missing argument $_") for qw< phase on >;
...
sub _read_meta_spec {
    ...
    $self->{meta_spec}{source} = join ("", @{$self->{meta_spec}{source}})
        if ref ($self->{meta_spec}{source}) eq 'ARRAY';
