...
sub build_package {
    ...
    my %should_use_mm = map +("perl/$_" => 1), qw( version ExtUtils-ParseXS ExtUtils-Install ExtUtils-Manifest );
    ...
    if ($has_build_pl && !exists $should_use_mm{$package}) {
        ...
        my $has_module_build = $self->run_command($build_dir, ['perl', '-MModule::Build', '-e1'], $opts)
            || $self->run_command($build_dir, ['perl', '-MModule::Build::Tiny', '-e1'], $opts);

        # If you have Module::Build, we can use it!
        if ($has_module_build) {
            @seq = $self->_build_pl_cmds($build_dir, $top_pkg_dir, $prefix, $use_prefix, $config_flags, $build_flags,
                $opts);
        } else {
            $log->warn('Defined Build.PL but can\'t load Module::Build. Will try Makefile.PL');
        }
    }

    if ($has_makefile_pl && !@seq) {
        @seq = $self->_makefile_pl_cmds($build_dir, $top_pkg_dir, $prefix, $use_prefix, $config_flags, $build_flags,
            $opts);
    }
...
sub _build_pl_cmds {
    ...
    return (
        ...
        # configure
        [$build_dir, ['perl', '-f', 'Build.PL', '--install_base', $prefix, @{$config_flags}], $opts],
...
sub _makefile_pl_cmds {
    ...
    return (
        ...
        # configure
        [$build_dir, ['perl', '-f', 'Makefile.PL', 'NO_PACKLIST=1', "INSTALL_BASE=$prefix", @{$config_flags}], $opts],
