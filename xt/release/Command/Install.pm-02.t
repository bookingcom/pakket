...
describe '"install" accepts STDIN' {
    ...
    tests 'install from STDIN' => sub {
        {
            my $command = 'echo "version" | ' . join (' ', $ctx{'app_run'}->@*) . ' install -f - -vvv 2>&1';
            my @output  = qx($command);
            match_any_item(\@output, 'matched to: perl/version=0.9924:1');
        }

        {
            my $command = 'echo "version" | ' . join (' ', $ctx{'app_run'}->@*) . ' install -f - -dvvv 2>&1';
            my @output  = qx($command);
            match_any_item(\@output, 'matched to: perl/version=0.9924:1');
        }

        {
            my $command = "cat $file | " . join (' ', $ctx{'app_run'}->@*) . ' install -f - -dvvv 2>&1';
